...
/***********************************************************
 cell重用机制说明：
 1. 当一个cell将要离开屏幕时，这个cell会进入重用池。重用池并不是什么特殊的东西，就是系统给他放在一边了。他只是单纯的放在一边了，不进行任何操作。
 2. 当一个cell将要进入屏幕的时候，会调用tableView:(UITableView *)tableView cellForRowAtIndexPath这个代理，执行其中的方法。
 ps. cell上不要布置太多的控件;tableView:(UITableView *)tableView cellForRowAtIndexPath里面添加子视图会导致来回滑动的时候cell会不停添加子视图；绘制出了什么问题，不要怪重用，问题一定在别的地方。
 **********************************************************/
