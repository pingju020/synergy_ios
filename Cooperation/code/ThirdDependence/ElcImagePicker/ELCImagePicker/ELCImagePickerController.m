...
- (id)initImagePickerWithSelectedImages:(NSArray *)selectedImages;
{
    self = [self initImagePicker];
    ELCAlbumPickerController *albumPicker = [self albumPicker];
    albumPicker.selectedImages = selectedImages;
    return self;
}
...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"ONLY_N_PHOTOS", nil), self.maximumImagesCount];
        title = nil;
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"ONLY_N_POTOS_SEND", nil), self.maximumImagesCount];
        message = [NSString stringWithFormat:@"最多选择%zi张图片", self.maximumImagesCount];
        NSString *okTitle = NSLocalizedString(@"OK", nil);
        [[[UIAlertView alloc] initWithTitle:title
                                    message:message
                                   delegate:nil
                          cancelButtonTitle:nil
                          otherButtonTitles:okTitle, nil] show];
    }
    return shouldSelect;
}
